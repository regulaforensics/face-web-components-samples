...
function App() {
  ...
  const componentRef = React.useRef<FaceVerifyWebComponent | null>(null);
  ...
  React.useEffect(() => {
    if (isOpen && componentRef.current) {
      componentRef.current.settings = {
        // you can add personId or externalId here
        verify: { personId: "" },
        customization: {
  ...
  React.useEffect(() => {
    ...
    containerCurrent.addEventListener("face-enroll", listener);
    return () => {
      containerCurrent.removeEventListener("face-enroll", listener);
    };
  }, []);
  ...
  return (
    <div style={containerStyle} ref={containerRef}>
      {isOpen ? (
        <face-verify ref={componentRef}></face-verify>
      ) : (
